...
void autosteerLoop()
{
  ...
  if (currentTime - autsteerLastTime >= LOOP_TIME)
  {
    ...
    if (watchdogTimer++ > 250) watchdogTimer = WATCHDOG_FORCE_VALUE;
            ...
            if (serialResetTimer++ > 102)
            {
                while (Serial.available() > 0) Serial.read();
                serialResetTimer = 0;
            }

            if (watchdogTimer > 102)
            {
...
void ReceiveUdp()
{
    ...
    if (len > 4)
    {
        ...
        if (autoSteerUdpData[0] == 0x80 && autoSteerUdpData[1] == 0x81 && autoSteerUdpData[2] == 0x7F) //Data
        {
            ...
            else if (autoSteerUdpData[3] == 0xEC) //236 EC Relay Pin Settings 
            {
                for (uint8_t i = 0; i < 24; i++)
                {
                    pin[i] = autoSteerUdpData[i + 5];
                }
            ...
            else if (autoSteerUdpData[3] == 0xEE)  //238 EE - machineConfig
            {
            ...
            else if (autoSteerUdpData[3] == 0xEF)  //239 EF machine data
            {
                uTurn = autoSteerUdpData[5];
                gpsSpeed = (float)autoSteerUdpData[6];//actual speed times 4, single uint8_t

                hydLift = autoSteerUdpData[7];
                tramline = autoSteerUdpData[8];  //bit 0 is right bit 1 is left

                relayLo = autoSteerUdpData[11];          // read relay control from AgOpenGPS
                relayHi = autoSteerUdpData[12];
...
void EncoderFunc()
{
  if (encEnable)
  {
    ...
    void SetRelays(void)
    {
        ...
        if (pin[0]) digitalWrite(9, relayState[pin[0] - 1]);
        if (pin[1]) digitalWrite(10, relayState[pin[1] - 1]);
        if (pin[2]) digitalWrite(26, relayState[pin[2] - 1]);
        if (pin[3]) digitalWrite(27, relayState[pin[3] - 1]);
        if (pin[4]) digitalWrite(24, relayState[pin[4] - 1]);
        if (pin[5]) digitalWrite(25, relayState[pin[5] - 1]);
    }
