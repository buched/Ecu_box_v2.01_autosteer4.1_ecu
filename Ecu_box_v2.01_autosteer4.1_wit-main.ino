...
HardwareSerial* SerialWit = &Serial3;  //Dual heading receiver 
HardwareSerial* SerialGPSTmp = NULL;
//HardwareSerial* SerialAOG = &Serial;

const int32_t baudAOG = 115200;
const int32_t baudGPS = 460800;
const int32_t baudWit = 230400;     // most are using Xbee radios with default of 115200
...
uint8_t Witrxbuffer[serial_buffer_size];    //Extra serial rx buffer
...
void setup()
{
  ...
  delay(500);
  SerialWit->begin(baudWit);
  SerialWit->addMemoryForRead(Witrxbuffer, serial_buffer_size);
  ...
  if (!useCMPS)
  {
   ...
   if (SerialWit->available() && !useCMPS && !useBNO08x)
  {
    useWit = true;
  }
  delay(100);
...
//    }
    ...
    if((systick_millis_count - READ_BNO_TIME) > REPORT_INTERVAL && useBNO08x)
    {
      ...
      readBNO();
        ...
        //Read BNO
    if((systick_millis_count - READ_BNO_TIME) > REPORT_INTERVAL && useWit)
    {
      READ_BNO_TIME = systick_millis_count;
      readWit();
    }
